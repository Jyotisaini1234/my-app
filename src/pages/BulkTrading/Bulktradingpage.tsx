  ...
  import {setSelectedClients, toggleSelectedClient,clearSelectedClients,clearResult, placeOrderForAll,} from '../../store/slice/bulkTradeSlice/bulkTradeSlice';
  ...
  import { ExchangeSelector } from '../../components/common/ExchangeSelector/ExchangeSelector';
  ...
  const SymbolSearch: React.FC<SymbolSearchProps> = ({exchange, clientCode, onSelect, initialSymbol,}) => {
...
const handleSelect = (item: SymbolSuggestion) => {
  const symbolName = (
    item.scripshortname && 
    item.scripshortname.trim() !== '' && 
    !/^\d+$/.test(item.scripshortname.trim()) 
  )
    ? item.scripshortname.trim()
    : (item.scripfullname?.trim() || '');
  ...
  console.log('[SymbolSearch] Selected:', { 
    symbolName, 
  ...
  interface UserDetails { user?: UserInfo; }
  ...
  export const BulkTradingPage: React.FC<UserDetails> = ({ user }) => {
    ...
    const loggedInAccount    = useMemo(
      () => activeClients.find(c => c.client_code === loggedInClientCode) ?? activeClients[0] ?? null,
      [activeClients, loggedInClientCode],
    );
  ...
  activeClients.forEach(c => {
    ...
    if (brokerKeys.length === 0) {
      console.warn(`[BrokerAccounts] No enabled brokers for client: ${c.client_code}`);
      return;
    }
    ...
    brokerKeys.forEach(brokerName => {
      ...
      const brokerUserId = (
        b?.user_id?.trim() || b?.userId?.trim() || c.user_id?.trim() || ''
      ).toUpperCase();

      if (!brokerUserId) {
        console.warn(`[BrokerAccounts] userId missing for client=${c.client_code} broker=${brokerName}`);
        return;
      }
      ...
      items.push({
        ...
        brokerName,
        userId:          brokerUserId,
        isAuthenticated: b?.is_authenticated ?? (c as any)?.is_authenticated ?? false,
        isMaster:        c.is_master,
        isActive:        c.is_active,
        // ✅ KEY CHANGE: userId:brokerName — backend exactly yahi expect karta hai
        selectionKey:    `${brokerUserId}:${brokerName}`,
      });
    });
  });
  return items;
}, [activeClients]);
    const allSelected = brokerAccounts.length > 0
      && brokerAccounts.every(a => selectedClients.includes(a.selectionKey));
    ...
    useEffect(() => {
      if (lastResult) dispatch(clearResult());
    }, [lastResult, dispatch]);

    const getBrokerInitials = (name: string) => name.slice(0, 2).toUpperCase();
    const setF = (key: string, val: string) => setForm(p => ({ ...p, [key]: val }));
const handleSymbolSelect = (symbol: string, token: string) => {
  const normalizedSymbol = symbol ? buildTsym(symbol, form.exchange) : '';
  setForm(prev => ({ 
    ...prev, 
    tradingsymbol: normalizedSymbol, 
    symboltoken: token 
  }));
};    const handleSelectAll = () =>dispatch(setSelectedClients(allSelected ? [] : brokerAccounts.map(a => a.selectionKey)));
    const handleExchangeChange = (exchange: string) => {
    setForm(prev => ({ ...prev, exchange, tradingsymbol: '',   symboltoken:   '', }));
    };

    const buildTsym = (symbol: string, exchange: string): string => {
    const s = symbol.toUpperCase();
    switch (exchange) {
      case 'NSE':
        return s.endsWith('-EQ') ? s : `${s}-EQ`;
      case 'BSE':
      case 'NSEFO': 
      case 'BSEFO': 
      case 'MCX':
  ...
  console.log('resolvedClients will be filtered from above');
    if (!loggedInClientCode) { showToast('No active client found.', 'error'); return; }
    if (!form.symboltoken || !form.tradingsymbol) { showToast('Please search and select a symbol first', 'error'); return; }
    if (!form.quantity || Number(form.quantity) < 1) { showToast('Please enter a valid quantity', 'error'); return; }
    if (selectedClients.length === 0) { showToast('Please select at least one account', 'error'); return; }

    const validKeys = new Set(brokerAccounts.map(a => a.selectionKey));
    const resolvedClients = selectedClients.filter(s => {
      if (!s.includes(':')) return false;
      const [userId, broker] = s.split(':', 2);
      return !!(userId && broker && validKeys.has(s));
    });

    if (resolvedClients.length === 0) {
      showToast('Selected accounts are invalid. Please reselect.', 'error'); return;
    }
    ...
    const payload = {
      ...
      tsym:  buildTsym(form.tradingsymbol, form.exchange),
      exch:    form.exchange,
    ...
    try {
      const result = await dispatch(placeOrderForAll(payload)).unwrap();
      const succeeded = result.successCount ?? 0;
      const failed    = result.failedCount  ?? 0;
      const results   = (result as any).results ?? {};
      const failedEntries = Object.values(results).filter((r: any) => r.status === 'ERROR');

      if (failed === 0 && succeeded > 0) {
        showToast(`✓ Order placed for ${succeeded} account${succeeded > 1 ? 's' : ''}`, 'success');
      } else if (succeeded === 0) {
        const msg = (failedEntries[0] as any)?.message || 'Order failed';
        showToast(`✗ ${msg}`, 'error');
      } else {
        showToast(`${succeeded}/${succeeded + failed} orders placed. Some failed.`, 'info');
      }
    } catch (err: any) {
              ...
              <div className="order-form">
                ...
                <div className="form-row">
                  <FormGroup label="Exchange">
                    <ExchangeSelector value={form.exchange} onChange={handleExchangeChange}/>
                  </FormGroup>
                </div>
                ...
                <div className="order-form__summary">
                  ...
                  <div className="order-form__summary-row"><span>Accounts</span><span>{selectedClients.length}</span></div>
                </div>

                <div className="order-form__actions">
                  <Button variant="accent"fullWidth loading={orderLoading} disabled={selectedClients.length === 0 || !loggedInClientCode ||  !form.symboltoken|| orderLoading }
                    onClick={handlePlaceOrder} icon={direction === 'BUY' ? <TrendingUp size={16} /> : <TrendingDown size={16} />} > Execute {direction} for {selectedClients.length} Account{selectedClients.length !== 1 ? 's' : ''}
                  </Button>
                ...
                <div className="client-selection__list">
                  {brokerAccounts.length === 0 && (
                    <div className="client-selection__empty">
                      No active broker accounts found. Make sure broker userId is configured.
                    </div>
                  )}
                  ...
                  {brokerAccounts.map(account => {
                    ...
                    return (
                      <div key={account.selectionKey} className={['client-selection__item', isSelected ? 'client-selection__item--selected' : '',  account.isMaster ? 'client-selection__item--master'   : '', ].filter(Boolean).join(' ')} onClick={() => dispatch(toggleSelectedClient(account.selectionKey))}  >
                        ...
                        <div className="client-selection__item-icon">{initials}</div>
                        <div className="client-selection__item-info">
                          <strong>{account.clientName}</strong>
                          <span className="client-selection__item-uid">
                            User ID: {account.userId}
                          </span>
                        </div>

                        <div className="client-selection__item-right">
                          <span className="client-selection__item-broker-badge" style={{ background: '#f0f0f0' }}>
